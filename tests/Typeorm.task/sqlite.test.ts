...
const coreConfigOverride = {
  config: { location: './tests/__fixtures__/config-sqlite' },
...
describe(TypeormTask, (): void => {
  it('db:create', async (): Promise<void> => {
    ...
    expect(SubProcess).toHaveRun('touch database')

    const cpuCount = os.cpus().length

    for (let i = 0; i < cpuCount; i++) {
      expect(SubProcess).toHaveRun(`touch database-test-${i + 1}`)
    }
  })
  ...
  it('db:drop', async (): Promise<void> => {
    ...
    expect(SubProcess).toHaveRun('rm database')

    const cpuCount = os.cpus().length

    for (let i = 0; i < cpuCount; i++) {
      expect(SubProcess).toHaveRun(`rm database-test-${i + 1}`)
    }
  })
})
